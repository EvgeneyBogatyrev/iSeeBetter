...
parser.add_argument('-m', '--model', default="/model/weights/netG_epoch_4_1.pth", help="Model")
...
parser.add_argument('-o', '--output', default='/model/results/', help="Location to save test results")
...
def eval():
    ...
    for batch in testing_data_loader:
        ...
        save_img(prediction.cpu().data, str(count + 1), True)
        '''
...
def save_img(img, img_name, pred_flag):
    ...
    # save img
    save_dir=os.path.join(args.output)
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
        
    save_fn = save_dir +'/frame'+ img_name.zfill(4) + '.png'
